 ...
 *
 * Architecture overview and block format: see turboquant_mi300x.h
 * Build instructions: bash build_mi300x.sh  (from kernels/hip/)
 *
...
// Gram-Schmidt orthogonalization of a seeded Gaussian matrix.
// NOTE: Uses an LCG + Box-Muller RNG — intentionally minimal to avoid LAPACK
// in the C library. For Python callers, turboquant_mi300x.py uses scipy.linalg.qr
// which produces a proper Haar-distributed orthogonal matrix.

static float lcg_randn(uint64_t *state) {
    // Box-Muller transform on two LCG samples → standard normal
