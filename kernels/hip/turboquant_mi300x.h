 ...
 *
 * Target:  gfx942 (CDNA3, MI300X/MI325X), ROCm 7.2, Wave64 (wavefront = 64 threads)
 * Build:   cd kernels/hip && bash build_mi300x.sh
 * Validate: ./tq_validate_mi300x  (9 tests, all must pass)
 *
 * Overview
 * --------
 * Implements TQ2/TQ3/TQ4 PolarQuant-style scalar quantization for KV cache
 * compression. Each 128-dim FP16 head vector (256 bytes) is compressed to a
 * bitplane block (36/52/68 bytes), achieving 4.92–7.11× compression.
 *
 * Compression pipeline per vector:
 *   1. L2-normalize:  x_unit = x / ||x||
 *   2. Rotate:        y = Π · x_unit  (128×128 random orthogonal matrix)
 *   3. Quantize:      idx[j] = nearest_centroid(y[j])  (Lloyd-Max codebook)
 *   4. Pack:          pack indices into b bitplanes via __ballot() — no LDS atomics
 *   5. Store:         [norm: float32][planes: b×16 bytes]
 *
 * Decompression reverses steps 4→1: unpack indices → centroid lookup → inverse
 * rotation → scale by stored norm.
 *
 * Bitplane format (enables zero-atomic packing via Wave64 ballot)
 * ---------------------------------------------------------------
 * Bit-plane p stores bit p of index[j] for all j=0..127, packed as two uint64:
 *   bytes [p*16..p*16+8)   = __ballot(bit_p_of_index[j]) over wave 0 (j=0..63)
 *   bytes [p*16+8..p*16+16) = __ballot(bit_p_of_index[j]) over wave 1 (j=64..127)
 *
 * Block sizes (head_dim=128):
 *   TQ2: 4 + 32 = 36 bytes  (7.11×)
 *   TQ3: 4 + 48 = 52 bytes  (4.92×)  ← primary production target
 *   TQ4: 4 + 64 = 68 bytes  (3.76×)
 *
 * Codebooks: Lloyd-Max optimal for the marginal distribution of uniformly random
 * unit vectors in R^128 (approximately N(0, 1/128)).
 * Source: ref/turboquant_codebooks.json, verified against ref/turboquant.py
 *
 * QJL correction (Algorithm 2 from the TurboQuant paper)
 * -------------------------------------------------------
 * For key vectors only: after TQ3 compression, compute 1-bit QJL residual
 * correction that captures the systematic MSE bias in key dot products.
 * The combined TQ3+QJL block achieves the full 3-bit paper quality.
 *
 * ABI note
 * --------
