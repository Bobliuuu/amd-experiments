"""
tq_backends/attention/backends/rocm_flash_attn.py — TurboQuant-enhanced ROCm Flash Attention Backend
...
GQA (e.g. Mistral 32 Q / 8 KV): fused decode **replicates compressed KV heads**
along the head axis (`expand_tq_compressed_for_gqa`, same semantics as FP16
`repeat_interleave` for SDPA).  Set ``VLLM_TQ_LOG_DISPATCH=1`` to log which path
runs per decode forward.

Integration:
  1. ``pip install vllm`` (ROCm build matching your torch), then
     ``bash scripts/install_turboquant_vllm_backend.sh`` to copy this file into
     site-packages ``vllm/attention/backends/``.
  2. Put ``amd-experiments/kernels`` on ``PYTHONPATH`` only (do not put repo root
     before site-packages or a stub may shadow PyPI ``vllm``).
  3. Call ``register_turboquant_rocm_backend()`` from ``benchmarks/vllm_turboquant_registry.py``
     before constructing ``LLM``. Run with:
...
Tested on: AMD Instinct MI300X (gfx942), ROCm 7.2 Primus, PyTorch 2.10,
...
_SDPA_PREFERENCE = os.environ.get("VLLM_TQ_SDPA_BACKEND", "flash").lower()
_DEBUG_PATHS = os.environ.get("VLLM_TQ_DEBUG_PATHS", "0") == "1"
_DEBUG_ONCE_KEYS: set[str] = set()
# Set VLLM_TQ_LOG_DISPATCH=1 to print every decode attention dispatch (path, head counts).
_LOG_DISPATCH = os.environ.get("VLLM_TQ_LOG_DISPATCH", "0") == "1"


def _log_tq_dispatch(msg: str) -> None:
    if _LOG_DISPATCH:
...
def _debug_once(key: str, message: str) -> None:
    ...
    print(f"[TurboQuantROCmAttention] {message}", flush=True)
...
def _run_sdpa(
    ...
    """
    Route SDPA through the preferred backend and fall back safely.
    """
    pref = _SDPA_PREFERENCE
    if hasattr(torch.nn, "attention") and hasattr(torch.nn.attention, "sdpa_kernel"):
        try:
            ...
            if pref in backend_map:
                _debug_once("sdpa_pref", f"SDPA preference={pref} via torch.nn.attention.sdpa_kernel")
                with sdpa_kernel([backend_map[pref]]):
                    return F.scaled_dot_product_attention(
                        q, k, v, attn_mask=attn_mask, scale=scale, is_causal=is_causal
                    )
        except Exception:
            pass
    _debug_once("sdpa_fallback", "SDPA preference unavailable, using default dispatcher")
    return F.scaled_dot_product_attention(q, k, v, attn_mask=attn_mask, scale=scale, is_causal=is_causal)
...
def tq3_gather_for_triton_batch(
    kv_cache: torch.Tensor,
    block_tables: torch.Tensor,
    seq_lens: List[int],
) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    Batch gather compressed KV for fused Triton decode.
    Returns padded tensors:
      k_planes, v_planes: (B, H, S_max, 48) uint8
      k_norms,  v_norms : (B, H, S_max) float32
    """
    ...
    k_planes = torch.zeros((batch_size, num_kv_heads, max_seq_len, TQ3_PLANES_BYTES), dtype=torch.uint8, device=device)
    v_planes = torch.zeros_like(k_planes)
    k_norms = torch.zeros((batch_size, num_kv_heads, max_seq_len), dtype=torch.float32, device=device)
    ...
    for i, seq_len in enumerate(seq_lens):
        ...
        cursor = 0
        for b in range(n_blocks):
            bi = block_tables[i, b].item()
            n_toks = min(block_size, seq_len - cursor)
            k_blk = kv_cache[0, bi, :, :n_toks, :]  # (H, n_toks, 52)
            v_blk = kv_cache[1, bi, :, :n_toks, :]
            k_planes[i, :, cursor:cursor + n_toks, :] = k_blk[:, :, 4:]
            v_planes[i, :, cursor:cursor + n_toks, :] = v_blk[:, :, 4:]
            k_norms[i, :, cursor:cursor + n_toks] = (
                k_blk[:, :, :4].contiguous().reshape(-1).view(torch.float32).view(num_kv_heads, n_toks)
            )
            v_norms[i, :, cursor:cursor + n_toks] = (
                v_blk[:, :, :4].contiguous().reshape(-1).view(torch.float32).view(num_kv_heads, n_toks)
            )
            cursor += n_toks
...
def expand_tq_compressed_for_gqa(
    ...
    """
    Repeat each KV-head row along the head axis so Triton sees MHA layout.

    Matches FP16 `_expand_kv_for_gqa` semantics: query head `h` uses KV head
    `h // gqa_ratio`.  Duplicates compressed bytes per logical head (same HBM
    traffic pattern as expanding FP16 KV for SDPA).

    Shapes: k_planes (B, H_kv, S, 48) -> (B, H_kv * ratio, S, 48), etc.
    """
    if gqa_ratio <= 1:
        return k_planes, k_norms, v_planes, v_norms
    return (
        k_planes.repeat_interleave(gqa_ratio, dim=1),
        k_norms.repeat_interleave(gqa_ratio, dim=1),
        v_planes.repeat_interleave(gqa_ratio, dim=1),
        v_norms.repeat_interleave(gqa_ratio, dim=1),
    )
...
class TurboQuantROCmAttentionImpl:
    ...
    def _forward_decode_decompress(
        ...
        _debug_once("decode_decompress", "Decode path=decompress+SDPA")
        _log_tq_dispatch(
            "[TQ_DISPATCH] path=DECOMPRESS_SDPA "
            f"num_q={self.num_heads} num_kv={self.num_kv_heads} gqa_ratio={self.gqa_ratio} "
            f"batch_decode={batch_size}"
        )
        outputs = []
        for i in range(batch_size):
            seq_len = int(seq_lens[i])
            q_i = query[i:i + 1].unsqueeze(2)
            k_i, v_i = tq3_gather_sequence(kv_cache, block_tables[i], seq_len, tq, dtype=torch.float16)
            k_i, v_i = self._expand_kv_for_gqa(k_i, v_i)
            out_i = _run_sdpa(q_i, k_i.to(q_i.dtype), v_i.to(q_i.dtype), scale=self.scale, is_causal=False)
    ...
    def _forward_decode_fused(
        ...
        GQA (num_q_heads > num_kv_heads): compressed KV is stored per KV head;
        we `repeat_interleave` planes/norms on the head axis (same semantics as
        FP16 `_expand_kv_for_gqa`) so the existing MHA-shaped Triton kernel runs.
        This duplicates compressed reads per Q-head group (benchmark / prod
        tradeoff); a future kernel can index KV heads without materializing.

        Falls back to _forward_decode_decompress only if the Triton entrypoint
        is unavailable.
        """
        triton_fwd = _get_triton_fwd()
        if triton_fwd is None:
            _debug_once(
                "decode_fused_fallback_triton",
                "Fused decode unavailable (Triton import failed) -> decompress+SDPA",
            )
            _log_tq_dispatch(
                "[TQ_DISPATCH] path=DECOMPRESS reason=no_triton "
                f"num_q={self.num_heads} num_kv={self.num_kv_heads} gqa_ratio={self.gqa_ratio}"
            )
        ...
        if seq_lens is None or block_tables is None:
            raise ValueError(
                "TurboQuantROCmAttentionMetadata requires seq_lens_decode and "
        ...
        if self.gqa_ratio > 1:
            k_planes, k_norms, v_planes, v_norms = expand_tq_compressed_for_gqa(
                k_planes, k_norms, v_planes, v_norms, self.gqa_ratio
            )
...
if __name__ == "__main__":
    ...
    print()
    print("Integration:")
    print("  1. bash scripts/install_turboquant_vllm_backend.sh")
